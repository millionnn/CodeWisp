...
class LspReferencesTool(Tool):
    ...
    def description(self) -> str:
        return (
            "Find references to the symbol at a position (0-based line/character). "
            "Use when tracing usages across the workspace."
        )
    ...
    def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            result = self._service.references(path, line, character)
...
def create_lsp_references_tool(
    workspace: Workspace,
    *,
    service: LSPService | None = None,
) -> LspReferencesTool:
    return LspReferencesTool(service or LSPService(workspace))
