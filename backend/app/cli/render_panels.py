...
def render_section(
    ...
    *,
    ...
    if _use_rich(output_fn):
        ...
        table.add_column("k", style="cw.key", width=12, no_wrap=True)
        ...
        if footer:
            ...
            body = Group(table, Text(), Text(f"  {footer}", style="cw.dim"))
        make_console().print(
            styled_panel(
                body,
...
def render_lsp_diagnostics(
    ...
    *,
    ...
    if not diags:
        if _use_rich(output_fn):
            make_console().print("[cw.ok]✓[/] [cw.value]LSP diagnostics clean[/]")
        else:
            output_fn("✓ clean")
        return
    ...
    if _use_rich(output_fn):
        ...
        make_console().print(
            styled_panel(
                body,
                title="[cw.warn]⚠[/] [cw.brand]LSP Diagnostics[/]",
                border=BORDER_WARN,
            )
        )
        return
    ...
    *,
    ...
    if _use_rich(output_fn):
        ...
        make_console().print(
            styled_panel(
                body,
                title="[cw.info]🌿[/] [cw.info]Branches[/]",
                border=BORDER_MUTED,
            )
        )
    else:
